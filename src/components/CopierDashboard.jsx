...
const DUMMY_TRADERS = [
    {
        name: 'John Doe',
        token: 'Fc4c2UrBO7VJhOW',
        profit: '+245.50 USD',
        trades: 150,
        success_rate: '85%'
    },
    // Add more dummy traders if needed
]

const TraderCard = ({ trader, onCopyClick, onStopCopy, isCopying, isProcessing }) => {
    return (
    ...
    useEffect(() => {
        ...
        const handleResponse = () => {
            ...
            if (wsResponse.msg_type === 'copy_start') {
                if (wsResponse.error) {
                    console.log('Showing error snackbar:', wsResponse.error.message)
                    setSnackbar({
                        isVisible: true,
                        message: wsResponse.error.message || 'Error starting copy trade',
                        status: 'fail'
                    })
                    setProcessingTrader(null)
                } else {
                    const trader = processingTrader
                    console.log('Showing success snackbar for trader:', trader.name)
                ...
                if (wsResponse.error) {
                    setSnackbar({
                        isVisible: true,
                        message: wsResponse.error.message || 'Error stopping copy trade',
                        status: 'fail'
                    })
                    setProcessingTrader(null)
                } else {
        ...
        <div className="max-w-6xl mx-auto p-6">
            <div className="mb-8">
                ...
                <Text size="lg" className="text-gray-600">
                    Find and copy successful traders to automate your trading strategy.
                </Text>
            </div>

            <div className="grid gap-6">
                {DUMMY_TRADERS.map((trader, index) => (
                    <TraderCard
                        key={index}
                        trader={trader}
                        ...
                        isProcessing={processingTrader?.token === trader.token}
                    />
                ))}
            </div>
